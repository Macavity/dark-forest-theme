...
1. **Install deps and build the `dist/` output:**
   ...
   ```sh
   bun install
   bun run build       # fonts + theme files → dist/
   ```
...
Source files (`theme.json`, `theme.css`, the scripts) are committed to git. `dist/` is a build artifact: regenerated by `bun run build`, gitignored, and the only directory the host ever sees.

| File | Purpose |
| --- | --- |
| `theme.json` | Source manifest (`id`, `modes`, `entry`, …) |
| `theme.css` | Source stylesheet — `@font-face` declarations + `--grove-*` token overrides |
| `scripts/build.ts` | Umbrella build: runs `build:fonts` + `build:theme` |
| `scripts/build-fonts.ts` | Copies the `.woff2` files out of `@fontsource` packages into `dist/fonts/` |
| `scripts/build-theme.ts` | Validates `theme.json`, copies the manifest + CSS into `dist/`. Hook point for minification. |
