...
The repo splits into two halves: `dist/` is *the theme* (what the host sees and what gets zipped for release), everything else is *dev/build scaffolding* (only used to produce `dist/`).

| File | Purpose |
| --- | --- |
| `dist/theme.json` | Theme manifest (`id`, `modes`, `entry`, …) |
...
| `scripts/validate-manifest.ts` | Validates `dist/theme.json` against the published manifest schema |
...
- A host app that loads themes via the `--grove-*` token namespace and a `theme.json` manifest. Minimum host version is declared in `dist/theme.json` (`minGroveVersion`).
