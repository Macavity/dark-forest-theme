...
1. **Install deps and build the `dist/` output:**
   ...
   The fonts are committed under `fonts/`; you only need `bun run build:fonts` to bump them after upgrading the `@fontsource/*` devDependencies.
...
Source files (`theme.json`, `theme.css`, `fonts/`, the scripts) are committed to git. `dist/` is a build artifact: regenerated by `bun run build`, gitignored, and the only directory the host ever sees.
...
| `scripts/build.ts` | Validates `theme.json`, copies the manifest + CSS + fonts into `dist/`. Hook point for minification. |
| `scripts/build-fonts.ts` | Refreshes `fonts/` from `@fontsource` packages. Run on scaffold or when bumping font versions; **not** part of `bun run build`. |
